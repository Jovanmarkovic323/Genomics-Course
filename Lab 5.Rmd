...
```{r}
ggplot(data=time_series_long_joined, aes(x=Date, y=Deaths)) +
...
```{r}
ggplot(data=time_series_long_joined_counts, aes(x=Date, y=Counts)) +
  geom_bar(stat="identity") +
  ggtitle("Worldwide Number of Confirmed Cases") +
...
time_series_long_joined %>% 
    group_by(Country_Region,Date) %>% 
    summarise_at(c("Confirmed", "Deaths"), sum) %>% 
    filter (Country_Region %in% c("Russia", "Spain", "Iran", "France","Italy","Germany", "Mexico", "India", "Brazil", "US")) %>% 
...
```
```{r, fig.height=8}
ggplot(data=Confirmed_State_9_13, aes(x=Province_State, y=Confirmed)) +
  geom_point() +
    
  facet_wrap(~Province_State, ncol=2, scales="free_y") +
ggthemes::theme_solarized_2() +
  ggtitle("Confirmed Cases per US State")
```
